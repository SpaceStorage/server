...
- Q: Which ClickHouse wire transports must the first release serve, and under what handler names? → A: Both transports as two handlers, each on its own entrypoint and both required in the first release: `clickhouse` (native binary protocol) and `clickhouse-http` (HTTP interface).
...
- Quorum `LOCAL_QUORUM` explicitly requested on a container that has no replicas in the client's region: rejected before execution with the requested level and the replica placement in the error; if `LOCAL_QUORUM` came from a default, it is clamped per FR-030 and the clamping is visible in the inspection mechanism.
...
- **FR-001**: The server MUST implement client-facing protocol handlers for PostgreSQL, Cassandra, Redis, Elasticsearch, ClickHouse, S3, and WebDAV, registered in the node's handler inventory under the names `postgresql`, `cassandra`, `redis`, `elasticsearch`, `clickhouse` (ClickHouse native binary protocol), `clickhouse-http` (ClickHouse HTTP interface), `s3`, and `webdav` — eight handlers for seven protocols. Both ClickHouse handlers are required in the first release; they share one driver behaviour (same type mappings, option handling, and namespace rule) and differ only in transport.
...
- **FR-038**: A request that uses a protocol feature or an operation combination the shared execution layer does not support MUST be rejected with the protocol's not-supported error form naming the operation, without partial execution.
...
- The entrypoint model (one port, one handler, optional transport encryption with referenced certificate material) is inherited from the runtime feature (`001`). This feature registers its seven handler names in that inventory and adds no second declaration form.
- "Different port per protocol" is treated as a MUST for this feature as the intent directs. A protocol family with more than one wire transport is modelled as one handler per transport, each on its own entrypoint. ClickHouse is served as `clickhouse` (native binary protocol) and `clickhouse-http` (HTTP interface), both required in the first release (confirmed in Clarifications, Session 2026-09-13); the handler inventory for this feature therefore has eight names for seven protocols.
- Compatibility baselines are the current widely deployed protocol versions of each system (the PostgreSQL wire protocol used by all current clients, the current CQL native protocol, current RESP versions for Redis, the current Elasticsearch REST API, the ClickHouse native and HTTP protocols, the S3 REST API including multipart upload, and WebDAV per the standard). Exact version ranges and the subset of each protocol's endpoints and statements covered in the first release are planning decisions; any endpoint or statement outside that subset returns the protocol's not-supported form, never a silent empty success.
...
- Namespace identity per protocol: protocols with a native selection step use it — database name (PostgreSQL, ClickHouse) and keyspace (Cassandra). Protocols without one — Redis, S3, WebDAV, Elasticsearch — bind the session to the namespace of the authenticated credential, and buckets, indices, keys, and paths name containers inside that namespace (confirmed in Clarifications, Session 2026-09-13). A session addresses one namespace; cross-namespace queries are out of scope for this feature. The credential-to-namespace binding is stored and managed by the role system of the tenancy and security feature.
- Quorum semantics (what `QUORUM`, `LOCAL_QUORUM`, and acknowledgement counts mean in terms of replicas, regions, and synchronous or asynchronous replication) are defined by the distribution and placement feature (`04`); this feature guarantees the value is carried on every query and validated against placement before execution, with explicit unsatisfiable levels rejected and default-sourced levels clamped (confirmed in Clarifications, Session 2026-09-13).

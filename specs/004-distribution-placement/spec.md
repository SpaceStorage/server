...
A cluster architect describes what the cluster physically is. Each node is given labels — `rack`, `az`, `region`, and any further key the architect invents, up to continent or planet. Each drive on a node is declared with its media kind (NVMe, SSD, HDD, or another), and those drive properties enrich the node's labels so that "this node has NVMe" is expressible as a placement constraint. In-memory storage is declared explicitly per node with a size and its own labels, and nodes without it simply do not declare it. The architect then asks the cluster what it sees: which nodes exist, what each is labelled with, what media and capacity each offers, and how many distinct values each label key has — which is exactly how many replicas can be spread apart by that key.
...
1. **Given** a node declaring labels `rack=rack1`, `az=az1`, `region=region1`, **When** the node joins the cluster, **Then** the cluster topology view reports the node with exactly those labels, and each label is visible to every other node.
2. **Given** two nodes labelled `rack1/az1/region1` and `rack2/az2/region1`, **When** the placement domains are requested, **Then** the cluster reports 2 distinct values for `rack`, 2 for `az`, and 1 for `region`, and states that these two nodes support anti-affinity by `rack` or `az` up to replication factor 2 and by `region` up to replication factor 1.
...
A service has users on two continents. The architect labels the nodes by `region` and `continent`, places each tenant's working data in the region of its users, and declares that replication inside a region is synchronous while replication to the far region is asynchronous with a lag the operator can see and bound. Local clients read and write at a local quorum level and never pay the long round trip; the remote copy exists for disaster recovery and for readers on the other continent, who are told how stale what they read may be. The same knobs, with more labels, describe a cluster spanning planets.
...
1. **Given** a two-region topology, **When** a container declares three replicas in `region=eu` replicated synchronously and two in `region=us` replicated asynchronously, **Then** the placement honours the per-region counts and modes, and the description reports each replica's region and its mode.
2. **Given** that container, **When** a client in `eu` writes at `LOCAL_QUORUM`, **Then** the write is acknowledged once a majority of the `eu` replicas have acknowledged it, the elapsed time does not include a round trip to `us`, and the `us` replicas receive the write afterwards.
3. **Given** the same container, **When** a client in `us` reads at `LOCAL_ONE`, **Then** it is answered from a `us` replica, and the answer's staleness is bounded and reported by the replica's current lag; **When** it reads at `QUORUM` across regions, **Then** the read crosses the link and the result is not stale.
...
5. **Given** the link between the two regions is cut, **When** clients keep writing in `eu` at `LOCAL_QUORUM`, **Then** those writes succeed, the backlog to `us` grows and is reported, and `us` readers are told how stale their region is; **When** the link is restored, **Then** the backlog drains, the lag returns below the threshold, and convergence is reported without operator action.
6. **Given** a level that requires an acknowledgement from every region (`EACH_QUORUM`) stated on a container whose remote replication is asynchronous and that has not opted into waiting, **When** the request is issued, **Then** it is refused before waiting on the remote region, naming the level, the asynchronous destination group, and that the container must declare waiting to honour the level; **Given** the same container with waiting declared, **When** `EACH_QUORUM` is stated, **Then** the write waits for a majority in every region.
7. **Given** a topology whose inter-region round trip is extremely long (the intent's interplanetary case), **When** containers are placed and replicated, **Then** no timeout, lag threshold, or retry budget is fixed in a way that assumes a low round trip: every one of them is configurable, and the documentation includes an example with a round trip measured in minutes.
8. **Given** the starter documentation, **When** an architect follows the multi-region example verbatim, **Then** the resulting cluster matches the described placement, modes, and quorum behaviour.
...
- **FR-001**: Every node MUST be describable with a set of labels as key/value pairs. Label keys MUST be operator-defined and open-ended; the cluster MUST NOT restrict topology to a fixed hierarchy. `rack`, `az`, and `region` MUST be documented as the starter keys, and deeper keys (for example `dc`, `continent`, `planet`) MUST work identically with no special casing.
...
- **FR-020**: A container MUST be able to declare anti-affinity by one label key or by an ordered list of keys; no two replicas MAY share a value of the selected key, and with an ordered list the spread MUST be maximised at the first key, then the next within it, and so on. The satisfied level MUST be reported per replica.
...
- **FR-028**: The cluster MUST support a per-query quorum level in the Cassandra-style vocabulary shared with the protocol drivers feature: `ONE`, `TWO`, `THREE`, `QUORUM`, `LOCAL_QUORUM`, `EACH_QUORUM`, `LOCAL_ONE`, `ALL`, and an explicit acknowledgement count. This feature owns what each level means in terms of replicas contacted and acknowledgements required.
...
- **FR-032**: `QUORUM` MUST mean a strict majority of the container's (or shard's) replicas; `LOCAL_QUORUM` MUST mean a strict majority of the replicas within the requester's locality domain; `EACH_QUORUM` MUST mean a strict majority within every locality domain that holds replicas; `LOCAL_ONE` MUST mean one replica within the requester's locality domain; `ALL` MUST mean every replica. The locality domain MUST be a label key, defaulting to `region` and configurable per cluster.
...
- **FR-045**: Every node MUST accept a request for any container in the cluster, whether or not it holds a replica, and MUST coordinate it: resolve placement, route to the shard and replicas, apply the quorum, and answer. The data path MUST be leaderless: each replica MUST be able to accept a write independently, and the coordinator MUST wait only for the acknowledgements the quorum level requires. No replica MAY be a mandatory write serializer. The control plane's datatype-level primary MUST own shared-datatype metadata, not per-write serialization.
...
- **FR-071**: Locality MUST be expressible as a label key so that "local" in `LOCAL_QUORUM` and `LOCAL_ONE`, the grouping for `EACH_QUORUM`, and the destination groups of per-group replication modes all derive from the same topology labels.
...
- **Label**: An operator-defined key/value pair on a node, a drive, or a memory pool. Declared labels and labels derived from drives are distinguished. Keys are open-ended; `rack`, `az`, `region` are the documented starter set.
...
- **Locality Domain**: The label key (default `region`) that defines what `LOCAL` means for quorum levels and destination grouping.
...
- **SC-010**: For a container with synchronous local and asynchronous remote replication, the measured local write latency at `LOCAL_QUORUM` stays within the local round trip and includes 0 remote round trips, in 100% of sampled requests; asynchronous lag is reported continuously in time and pending writes.
...
- **Quorum vocabulary and precedence** are shared with the protocol drivers feature (`002`), which already specifies how each protocol carries the level and the timeout, the clamping rule, and the inspection surfaces. This specification adds the container level to the precedence chain and owns the meaning and satisfiability arithmetic of each level. The global defaults (write two acknowledgements, read one) come from the constitution.
...
- **Locality** is just a label key, defaulting to `region`, so `LOCAL_QUORUM`, `EACH_QUORUM`, and destination grouping need no separate topology concept. A cluster that wants continent-level locality changes one setting. `EACH_QUORUM` on a container with asynchronous remote replication is refused unless the container opts into waiting (confirmed in Clarifications, Session 2026-09-14).

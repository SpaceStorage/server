...
- Session 2026-09-15: topology ladder (cluster-declared subsequence of reserved farness keys); `planet` is a label not a voting/HLC domain; write-ack arithmetic and `LOCAL_*` owned with `12`; default RF≥2 anti-affinity = finest ladder key; leaderless in source domain, forward from followers. **Re-run `/speckit-plan` for this feature** — contracts still describe locality as a `region` label key.

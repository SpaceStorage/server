...
- Decisions resolved in the Clarifications section (Session 2026-09-13): (1) omitting an admin handler declaration is a startup error, explicit disable is allowed; (2) every listening port is an entrypoint with exactly one handler; (3) TLS may be chosen per entrypoint with referenced, never inlined, certificate material; (4) buffer capacities and drain timeout are live-reloadable, thread count and entrypoints are restart-required; (5) default thread count is one per available core, minimum 1.
- Session 2026-09-15 (intent + grill): every enabled entrypoint MUST declare `tls` or `plaintext;` (omit is a startup error; TLS is still not globally mandatory); `internode`/`replication` always listen once registered; drain also blocks new replica placements; fsync off worker threads.
